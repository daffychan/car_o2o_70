...
<script>
import { getStatement} from "@/api/appointment/statement";
import {addStatementItem,listStatementItem,payStatement} from "@/api/appointment/statement_item";
import { listItem } from "@/api/appointment/item";
export default {
  name: "StatementItem",
  dicts: ["bus_service_type", "bus_package_type"],
  data() {
    return {
      //优惠价格
      discountAmount: 0,
  ...
  methods: {
    ...
    pay() {
      ...
      payStatement(data).then(response=>{
        this.$modal.msgSuccess(response.msg);
        this.getDetail();
      })
    },
